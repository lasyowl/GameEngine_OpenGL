...
void Display::DrawOffscreen_Water() {
	...
	gameObject->render_sphere->Draw(VP_sky);
...
void Display::DrawOffscreen_Shadowmap() {
	...
	gameObject->render_models_animated->DrawModels(VP_shadow, gf.CalcTimeElapsed());
...
void Display::DrawTerrain(Terrain *terrain, const mat4 &VP) {
	...
	for (int i = 0; i < 4; i++) {
		gameObject->terrain->child[i].FrustumTest(camera->GetPos());
	}
...
void Display::DrawTerrain_R(Terrain *terrain) {
	if (flags_io->sceneState == SCENE_TERE) {
		gameObject->render_terrain->SetIBO(terrain->GetIbo(0), terrain->numIndex[0]);
		gameObject->render_terrain->Draw();
	}
	else {
		if (terrain->inbound == DRAW_ALL) {
			gameObject->render_terrain->SetIBO(terrain->GetIbo(0), terrain->numIndex[0]);
			gameObject->render_terrain->Draw();
		}
		else if (terrain->inbound == DRAW_CHILD) {
			for (int i = 0; i < 4; i++) {
